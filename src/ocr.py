...
try:
    ...
    _issue(f"Failed to initialize EasyOCR Reader. It may need to be reinstalled. Error: {e}")
    reader = None
...
def scan_screen(
    ...
    if not reader:
        ...
        raise RuntimeError("EasyOCR Reader is not initialized. Please check the installation.")
...
def try_text(region: Rect, query: str, window_title: str = "Roblox") -> bool:
    ...
    return any(jaccard_similarity(query.lower(), result.lower()) > .75 for result in results)